...
export const CreateHabit = async (req, res) => {
	try {
		...
		const { title, description, category, logo, frequency, reminder, priority } = req.body;
		...
		const NewHabit = await newHabit.save();
		if (!NewHabit) {
			return res.status(400).json({
				success: false,
...
export const GetMyHabits = async (req, res) => {
	try {
		...
		let errorReference = "HABITS_FETCH_FAILED";
		let errorDetails = error.message;
...
export const UpdateMyHabit = async (req, res) => {
	try {
		...
		const Updating = await Habit.updateOne({ _id: habitId, user_id: userId }, { $set: UpdateMyHabit });
		if (Updating) {
			return res.status(200).json({
				success: true,
	...
	try {
		...
		if (!habitId) {
			return res.status(400).json({
				...
				details: "Habit ID must be provided either in URL parameters or request body"
			});
		}
